...
export default class Navbar extends React.Component {
  ...
  render() {
    ...
    let navigationClass
    if (
      (this.state.currentScroll < 66 && !this.state.isNavigationVisible) ||
      this.state.currentScroll < 3
    ) {
      navigationClass = 'navigation'
    } else if (
      (this.state.currentScroll < 120 && !this.state.isNavigationVisible) ||
      this.state.currentScroll < 6
    ) {
      navigationClass = 'navigation navigation--fixed'
    } else {
      navigationClass =
        'navigation navigation--fixed navigation--animate' +
        (this.state.isNavigationVisible
          ? ' navigation--visible'
          : ' navigation--hidden')
    }
    ...
    let btnClass =
      'btn btn--nav' + (this.state.isMenuOpen ? ' btn--active' : '')
    let brandClass =
      'navbar__logo text-2xl brand' +
      (this.state.isMenuOpen ? ' brand--white' : '')
    let navClass =
